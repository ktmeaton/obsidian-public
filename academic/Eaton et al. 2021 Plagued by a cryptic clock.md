...
![The phylogenetic relationship between the Third Pandemic (1.ORI) and the ancestral *Intermedium* population (1.IN). The subtree was extracted from the maximum-likelihood phylogeny. Branch lengths are scaled by genetic distance in substitutions per site. The phylogeny is annotated with node dates from the Bayesian analysis, where each population was assessed independently. MRCA dates include the 95% highest posterior density (HPD) interval. Grey branches indicate outliers, as defined by the 90% confidence interval of external branch lengths from all populations.](https://raw.githubusercontent.com/ktmeaton/plague-phylogeography-projects/451ffbd/main/auspice/all/chromosome/full/filter5/ml/divtree_1.IN_1.ORI.png){#fig:divtree_1.IN_1.ORI width=80%}
...
After excluding populations with no detectable signal and node-dating conflicts, we identified five populations with potentially informative rates and dates. These include the Third Pandemic (1.ORI), *Medievalis* (2.MED), the Second Pandemic (1.PRE), the First Pandemic (0.ANT4), and the Bronze Age (0.PRE). These populations have several shared traits which make their temporal estimates particularly robust and of great historical significance. 
